...
modality = "MRI"
anatomy = "kidney"  # anantomy + dataset name
# img_name_suffix = "_0000.nii.gz"
img_name_suffix = ".nrrd"
...
prefix_data = 'train'  # train, valid or test
nii_path = f"data/KidneyData/images/"  # path to images
gt_path = f"data/KidneyData/labels/"  # path to labels
# npy_path = f"data/KidneyData/npy_{image_size}_{prefix_data}/" + prefix[:-1]  # output path
output_path = f"data/KidneyData/"  # output path
npy_path = f"{output_path}/npy_{image_size}_{prefix_data}/" + prefix[:-1] # output npy path
os.makedirs(join(npy_path, "gts"), exist_ok=True)
os.makedirs(join(npy_path, "imgs"), exist_ok=True)

valid_case = ''  # specify the case for validation data
test_case = 'case1'  # specify the case for testing data
...
WINDOW_LEVEL = 300  # only for CT images
WINDOW_WIDTH = 500  # only for CT images

# %% save preprocessed images and masks as npz files
for name in tqdm(names[:]):
    if prefix_data == 'train':
        if (valid_case != '' and valid_case in name) or (test_case != '' and test_case in name):
            continue
    if prefix_data == 'valid':
        if valid_case not in name and valid_case != '':
            continue
    if prefix_data == 'test':
        if test_case not in name and test_case != '':
            continue
    # if valid_case in name:
    #     prefix_data = 'valid'
    #     npy_path = f"{output_path}/npy_{image_size}_{prefix_data}/" + prefix[:-1] # output npy path
    ...
    if len(z_index) > 0:
        ...
        for i in range(img_roi.shape[0]):
            if i < 6 or i > 14:
                # filter the slices that can not see the kidney
                continue
            img_i = img_roi[i, :, :]
            ...
            resize_img_skimg = transform.resize(
                ...
                order=3, # order=3: Bicubic interpolation
                preserve_range=True,
                mode="constant",
                anti_aliasing=True,
            )
